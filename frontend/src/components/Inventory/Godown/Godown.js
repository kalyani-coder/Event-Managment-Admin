...
const GodownInventory = () => {
  ...
  const handleAddStock = async () => {
    try {
      ...
      const stockExists = selectedVendorStocks.some(
        (stock) => stock.Stock_Name === newStockName
      );

      if (stockExists) {
        alert(
          "Stock with the same name already exists for the selected vendor"
        );
  ...
  const fetchVendorStocks = async (vendorId) => {
    try {
      const response = await fetch(
        `http://localhost:8888/api/inventory-stocks/vendor/${vendorId}`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleConfirmEditQuantity = async () => {
    try {
      ...
      await fetch(
        `http://localhost:8888/api/inventory-stocks/${selectedStockId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
