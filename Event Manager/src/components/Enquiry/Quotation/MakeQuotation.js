...
function QuotationForm() {
    ...
    const addStock = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:5000/api/quotationinfo/${customerId}`);
           
            await axios.patch(`http://localhost:5000/api/quotationinfo/${customerId}`, {
                requirements: [...response.data.requirements, ...requirementsData],
            });
            alert("Stock added successfully!");
        } catch (error) {
            // If customer has no existing data, make a POST request to create a new entry
            ...
            await axios.post("http://localhost:5000/api/quatationinfo", data);
